...
//            $dicom->parse(array(array(0x0010, 0x0010)));
            ...
            $data = [
                'PatientName' => $dicom->value(0x0010, 0x0010),
                'PatientID' => $dicom->value(0x0010, 0x0020),
                'PatientBirthDate' => $dicom->value(0x0010, 0x0030),
                'PatientSex' => $dicom->value(0x0010, 0x0040),
                'PatientAge' => $dicom->value(0x0010, 0x1010),
                'BodyPartExamined' => $dicom->value(0x0018, 0x0015),
                'StudyInstanceUID' => $dicom->value(0x0020, 0x000D),
                'Modality' => $dicom->value(0x0008, 0x0060),
                'InstitutionName' => $dicom->value(0x0008, 0x0080),
                'ReferringPhysicianName' => $dicom->value(0x0008, 0x0090),
            ];


            echo 'Patient  if exists: '.$data['PatientName']."\n<br>";
            echo 'PatientID  if exists: '.$data['PatientID']."\n<br>";
            echo 'PatientBirthDate  if exists: '.$data['PatientBirthDate']."\n<br>";
            echo 'PatientSex  if exists: '.$data['PatientSex']."\n<br>";
            echo 'PatientAge  if exists: '.$data['PatientAge']."\n<br>";
            echo 'BodyPartExamined  if exists: '.$data['BodyPartExamined']."\n<br>";
            echo 'StudyInstanceUID  if exists: '.$data['StudyInstanceUID']."\n<br>";
            echo 'Modality  if exists: '.$data['Modality']."\n<br>";
            echo 'InstitutionName  if exists: '.$data['InstitutionName']."\n<br>";
            echo 'ReferringPhysicianName  if exists: '.$data['ReferringPhysicianName']."\n<br>";
            var_dump($data);
...
//            echo 'Patient name should be empty here: '.$dicom->PatientName."\n<br><br><br><br><br>";
        ...
        catch (DicomParser\Nanodicom_Exception $e)
        {
            echo 'File failed. '.$e->getMessage()."\n<br>";
        }
    ...
    */
    ...
    // 4) Load only given tags. Dump it and print certain tags. Load 'dumper' directly. [ ok ]
/*
        try
        {
...
*/
    ...
    */
    ...
    // 8) Load 'dumper' directly. Dump output is in html format. [ ok ]
/*
    try {
...
*/
    ...
    /*
        try {
            ...
            $tags = array(
                ...
                array(0x0008, 0x0050, '{date|Ymd}'),         //test reza
                ...
                array(0x0010, 0x21B0, 'reza'),            // Patient Date of Birth
            );
            $dicom = Nanodicom::factory($filename, 'anonymizer');
    ...
    */
    ...
    // 20) Gets the images from the dicom object if they exist. This example is for gd [ ERROR ]
    /*
        try {
